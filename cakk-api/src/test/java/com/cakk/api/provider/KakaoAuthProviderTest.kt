...
internal class KakaoAuthProviderTest : MockitoTest() {
	...
	fun getProviderId() {
		...
		// when
		val result = kakaoAuthProvider.getProviderId(idToken)
	...
	fun getProviderId2() {
		// given
		doThrow(CakkException(ReturnCode.EXTERNAL_SERVER_ERROR)).`when`(kakaoAuthClient).getPublicKeys()
