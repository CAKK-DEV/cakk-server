...
class KeywordServiceTest extends ServiceTest {
	...
	void findTopSearched1() {
		// given
		final TopSearchedListRequest request = getConstructorMonkey().giveMeOne(TopSearchedListRequest.class);
		final List<String> keywordList = getConstructorMonkey().giveMe(String.class, 3);

		doReturn(keywordList).when(keywordRedisRepository).findTopSearchedLimitCount(request.count());
		...
		verify(keywordRedisRepository, times(1)).findTopSearchedLimitCount(request.count());
	}
